...
- **Application entry**: `AppDelegate.swift`; b14 also sequences accepted WebKit warm-up before installing the product root.
...
- **Production conversation owner**: `Conversation/ConversationFeature.swift` / `ConversationRepository`; owns production list summaries, selected conversation identity, loaded detail/current visible branch, manual recovery and current selected-detail operation generation.
- **Test roots**: None yet.
...
- **Current validation level**: Foundation, embedded Google/WebKit auth architecture, Plus/personal account context, b7 diagnostic list/detail, b9 production native read, b10 recovery core, b12 public WebKit cold-start warm-up + centered sync feedback, and b13 immediate list initiation/stale-generation rejection have real-device evidence on iPhone/iOS17. b13 compact navigation failed and exposed HTTP429 on overlapping replacement detail requests. b14 compact shell correction is Code+CI+Artifact only and awaits real-device confirmation.
...
- **Active candidate**: `0.1.0 (14)` / `DEV-conversation-recovery-0.1.0-b14`; product/config head `82d96bf085dbee3877bcb16e27bbf69f4dc0990f`; run `33000566633`; artifact `9618410313`; IPA SHA `b9100deb1d59b8ce22e15e72f766f0313be2903ec96ed2cda3d397986ba89182`.
...
- b9 accepted the production shell/list/two distinct detail reads/current visible branch on iPhone/iOS17.
...
- b13 proved the first list load can begin immediately after warm-up; its tested list returned HTTP200 28/29 after `22005.52 ms`. The user's much longer wait to reach it was caused by compact shell/navigation presentation, including duplicate sidebar controls.
- b13 also proved the selected-detail freshness generation rejects an older completion, while concurrent manual replacement requests can hit HTTP429. That overlap remains unresolved in b14.
- b14 changes only launch/root/split presentation to start compact on the conversation list and rely on one native navigation owner.
