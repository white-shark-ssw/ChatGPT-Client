...
7. Before multi-conversation, production send/stream, background-completion or TrollStore true-background work, also read `CLIENT_ARCHITECTURE_GAP_REVIEW.md` plus the relevant `MULTI_CONVERSATION_STATE_PLAN.md` / `BACKGROUND_EXECUTION_PLAN.md`. Before `DEV-send-stream` specifically, also read `SEND_STREAM_PREFLIGHT.md`; it records current evidence gates for Send/new-chat/stream/Stop, Sentinel-safe evidence acquisition, pending-to-authoritative identity handoff, response ownership, reasoning/follow-tail, diagnostics and Runtime acceptance. These documents contain pre-send concurrency, async-freshness, account-scope and background active-response-set invariants that are not safe to rediscover ad hoc.
...
11. Before any **existing-account hybrid Web Send background/lock resilience** experiment, also read `HYBRID_WEB_BACKGROUND_RESILIENCE_PLAN.md`. It owns the current go/no-go gate for preserving or automatically recovering the visible official-Web reasoning/stream surface on the primary iPhone/iOS17 TrollStore runtime. Do not infer that keeping only the main app process alive proves WebKit stream continuity.
