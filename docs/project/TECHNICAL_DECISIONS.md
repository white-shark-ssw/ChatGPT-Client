...
- **Memory boundary**: b19 real-device task-VM evidence reached 8 residents with physical footprint about 16.3–78.1 MiB and generally 55–65 MiB during repeated 8-resident switching. That evidence does not justify an urgent or arbitrary normal LRU capacity. Existing memory-warning trimming remains the evidence-backed eviction behavior; if stronger future pressure/headroom evidence appears, normal LRU belongs to the owning future Work.
- **Runtime evidence**: b17 accepts resident return, hidden completion, same-target coalescing, Sync A->B->A rejoin and rapid overlap. b18 accepts independent historical-scroll isolation/preservation plus resident/coalescing regressions. b21 accepts the selected-title lifecycle correction and exact same-target ordinary-load -> Reload replacement-under-load: old generation cancelled, replacement HTTP200, unrelated conversation remained independent, and return while Reload remained active coalesced onto the same generation without duplicate request or stale overwrite.
- **Merge evidence**: exact Runtime Candidate `DEV-multi-conversation-state-0.1.0-b21` uses product/config source `6b50ead167bfde305d2ad58dd16fee6edaabf597`, Run `33070183417`, Artifact `9645439329`. PR #23 merge-view Run `33093117645` / Job `98590935774` passed on merge view `0520f118d4ada5eacfbac4ff444d9572e322efe1`; PR #23 then merged at `2057a6241839afabeaf9b81c9daea24d3a0978f6`. Merge-view Artifact is CI evidence only and does not replace the exact Runtime Candidate.
- **Conditional boundaries after closure**: natural failed-resident navigation remains Runtime-unverified until a natural terminal failure exists; supported account-switch purge remains Runtime-unverified until a real supported route exists; non-personal workspace identity remains Unknown/Unverified; missing-anchor-message discard remains conditional Runtime-unexercised. These are not claimed Passed and are not reasons to manufacture unsupported behavior.
- **Rejected**: stale operation context re-adopting account scope; selection-driven cancellation; separate screen repositories; retained VC/cell cache; navigation-triggered reload; speculative retry/timer/watchdog/global rate limiter; arbitrary LRU capacity; persistent chat-body cache without a separate contract.
