...
| `DEV-conversation-recovery-0.1.0-b11` | `DEV-conversation-recovery` | `0.1.0 (11)` | `dev/conversation-recovery-20260826` / PR #10 open | **`8ab510265d891fc6b7eb048f03a5a683ec94b44b`** | **Code written + static/source diff reviewed; CI not run** | Expected `ChatGPTClient-0.1.0-b11-dev-conversation-recovery.ipa`; **not produced** | Runtime not tested. Adds only non-blocking sync status/result feedback plus build/candidate metadata; no repository/protocol/auth semantic change. | **Active candidate / CI blocked by available GitHub tooling** |
...
- Build/candidate metadata is `0.1.0 (11)` / `DEV-conversation-recovery-0.1.0-b11`; expected artifact name is `ChatGPTClient-0.1.0-b11-dev-conversation-recovery.ipa`.
- Current exact product/config source is `8ab510265d891fc6b7eb048f03a5a683ec94b44b`.
- No Actions run exists for b11. Old failed run `32985845398` tested old `bdb18407...` and must not be used as b11 evidence. Available GitHub tooling could not issue `workflow_dispatch`, and connector-originated ref/path/PR reopen changes did not emit Actions. Therefore b11 is Code+static-review only, not CI/Artifact/Runtime.
