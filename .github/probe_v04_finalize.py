...
CHECKPOINT_BLOCK = '''## Official iOS Probe v0.4 package-ready research override — 2026-09-04
...
STATE_BLOCK = '''## 2026-09-04 — official iOS Probe v0.4 package ready / task-resume late-join gate
...
EVIDENCE = '''# DEV-send-stream — official iOS Probe v0.2 Inconclusive / v0.4 task-resume gate — 2026-09-04
...
BASE_WORKFLOW = '''name: Research Official iOS Realtime Probe
...
s = cp.read_text()
if CHECKPOINT_BLOCK.splitlines()[0] not in s:
    s = s.replace('# DEV-send-stream\n\n', '# DEV-send-stream\n\n' + CHECKPOINT_BLOCK, 1)
    cp.write_text(s)

ps = Path('docs/project/PROJECT_STATE.md')
s = ps.read_text()
if STATE_BLOCK.splitlines()[0] not in s:
...
Path('.github/workflows/research-official-ios-realtime-probe.yml').write_text(BASE_WORKFLOW)
for name in [
    '.github/probe_v04_apply.py',
    '.github/probe_v04_finalize.py',
    '.github/workflows/official-ios-probe-v04-apply.yml',
