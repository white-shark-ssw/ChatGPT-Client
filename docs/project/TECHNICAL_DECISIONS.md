...
- **Status**: Confirmed architectural direction; b17 core runtime partially accepted / Work Active
- **Date**: 2026-08-27
...
- **Validation boundary**: b16 is rejected historically. Exact b17 source `bc69d58b...` passed static/CI/Artifact identity and the supplied iPhone/iOS17 runtime accepts resident return, hidden completion, same-target coalescing, Sync A->B->A rejoin, and rapid overlap without HTTP429 in that export. Reload/failure/account-switch/LRU gates remain open, and the Work is not Stable/Frozen.
...
- **Status**: User-confirmed product requirement; ordinary anchor defect Runtime reproduced on b17; active-response follow-tail Runtime pending Send/Stream
- **Date**: 2026-08-27
...
- **Response-owner boundary**: pre-send scroll work may establish the semantic presentation model, but whether a response is active/terminal must come from the future authoritative per-conversation Send/Stream response owner. Do not invent a separate UI `isStreaming` authority merely to implement follow-tail early.
- **Runtime gates**: b17 directly reproduced ordinary A ~10% -> B scroll -> A position drift. Future Send/Stream must separately prove (1) A active at bottom -> B -> A completes hidden -> return A at latest bottom, and (2) A active -> user scrolls upward -> B -> return A at preserved reading anchor.
