...
| `DEV-conversation-recovery-0.1.0-b10` | `DEV-conversation-recovery` | `0.1.0 (10)` | `dev/conversation-recovery-20260826` / PR #10 open | **`89129913cb29a35db9dec7a6d5670d1b3b76bc23`** | **Code written + CI passed + Artifact produced** | **Run `32982836557`; artifact ID `9612167843`; IPA `ChatGPTClient-0.1.0-b10-dev-conversation-recovery.ipa`; IPA SHA-256 `6e600f829fa24cdeb705e9ab104ebb780a8c70dd06871285d06fa30521aecb7e`; ZIP digest `sha256:5a4818f2ea10569092e4144630372d2fecc62a2fcc87d03010ad2116947d224c`** | Runtime/manual/real-device recovery behavior not tested yet. Package inspection confirms version `0.1.0`, build `10`, candidate `DEV-conversation-recovery-0.1.0-b10`, source `89129913cb29`, minimum OS `14.0`, arm64. | **Active test candidate / runtime pending** |
...
- `同步最新消息` fetches the selected conversation's current server detail, preserves the existing loaded detail if the fetch fails, and replaces/reconciles the authoritative selected detail on success. It never resends/regenerates.
- `重载当前会话` explicitly clears the selected conversation detail before issuing a fresh detail request, then rebuilds the visible state from the returned server-backed current branch. The terminal `重新加载` action uses this full-reload path.
- Recovery diagnostics record action spans, previous/current visible counts, added/removed/changed counts and local-state transitions while continuing to omit raw conversation IDs/message bodies/auth secrets.
- The detail overflow menu exposes `同步最新消息` and `重载当前会话`; no automatic retry, timer, watchdog, fallback endpoint or resend chain was added.
- CI run `32982836557` passed for exact product/config source `89129913cb29a35db9dec7a6d5670d1b3b76bc23`; artifact `9612167843` was produced. The later PR-head checkpoint/docs commits do not change product/config source.
- Downloaded artifact inspection verified the sidecar SHA-256 and embedded Info.plist identity: `0.1.0 (10)`, candidate `DEV-conversation-recovery-0.1.0-b10`, source `89129913cb29`, minimum OS `14.0`; executable is arm64.
- Runtime/manual/real-device behavior remains Unverified. CI and Artifact do not prove that sync/reload interactions are accepted on device.
