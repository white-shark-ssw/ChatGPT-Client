...
Development completion moves durable conclusions to long-term docs, records final validation/candidate in the index, and removes only that task's current checkpoint. Rules completion moves durable rules to permanent files and resets only the rules checkpoint to Idle.
