...
static NSString * const RPTBatchVersion = @"0.8.1";
...
static void RPTBatchWriteEvent(NSString *name, NSDictionary *fields) {
    if (!name.length || !RPTBatchLogQueue) return;
...
static NSMutableDictionary *RPTBatchTaskFields(NSURLSessionTask *task) {
    ...
    NSArray<NSString *> *parts = RPTBatchPathParts(request.URL.path ?: @"");
    if (parts.count >= 3) fields[@"conversationHash"] = RPTBatchHashString(parts[2]);
...
static BOOL RPTBatchIsTargetTask(NSURLSessionTask *task) {
    if (!task) return NO;
    NSURLRequest *request = task.currentRequest ?: task.originalRequest;
    return RPTBatchIsConversationDetailURL(request.URL);
}
...
static void RPTBatchScanData(NSData *data, NSMutableData *tail, BOOL *keyPresent, NSString **status) {
    ...
    if (data.length >= RPTBatchTailBytes) [tail setData:[data subdataWithRange:NSMakeRange(data.length - RPTBatchTailBytes, RPTBatchTailBytes)]];
    else {
...
static void RPTBatchRecordCallback(NSURLSessionTask *task, SEL selector, NSDictionary *extra) {
    if (!RPTBatchIsTargetTask(task)) return;
    NSMutableDictionary *fields = RPTBatchTaskFields(task);
    @synchronized (task) {
        NSUInteger sequence = [objc_getAssociatedObject(task, &RPTBatchCallbackSequenceKey) unsignedIntegerValue] + 1;
...
static void RPTBatchTaskDispatchData(id self, SEL _cmd, dispatch_data_t dispatchData, id completionHandler) {
    ...
    if (!original) return;
    ((void (*)(id, SEL, dispatch_data_t, id))original)(self, _cmd, dispatchData, completionHandler);
}
...
static void RPTBatchConnectionDidReceiveData(id self, SEL _cmd, id connection, id data, id completionHandler) {
    ...
    IMP original = RPTBatchOriginalIMP(self, _cmd);
    if (!original) return;
    ((void (*)(id, SEL, id, id, id))original)(self, _cmd, connection, data, completionHandler);
}
...
static BOOL RPTBatchInstallExactHook(Class cls, SEL selector, IMP replacement, NSString *expectedEncoding) {
    ...
    if (!method) { RPTBatchLogSkippedSurface(cls, selector, expectedEncoding, @"missing"); return NO; }
    const char *rawTypes = method_getTypeEncoding(method);
...
static void RPTBatchInstallKnownTaskHooks(void) {
    Class cls = NSClassFromString(@"__NSCFLocalDataTask");
    if (!cls) { RPTBatchWriteEvent(@"batch.task_class_missing", @{ @"class": @"__NSCFLocalDataTask" }); return; }
    ...
    RPTBatchWriteEvent(@"batch.hooks_installed", @{ @"taskClass": NSStringFromClass(cls) ?: @"", @"installedSelectors": installed });
}

__attribute__((constructor)) static void RPTBatchInitialize(void) {
    @autoreleasepool {
        ...
        RPTBatchWriteEvent(@"batch.loaded", @{ @"bundleID": NSBundle.mainBundle.bundleIdentifier ?: @"", @"logFile": RPTBatchLogName, @"startupMode": @"no_resume_swizzle" });
        RPTBatchInstallKnownTaskHooks();
    }
}
