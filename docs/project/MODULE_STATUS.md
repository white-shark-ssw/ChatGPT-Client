...
| Native application shell | **Accepted Candidate** | `AppDelegate.swift`, `RootViewController.swift`; `DEV-native-read-path-0.1.0-b9` | Split sidebar/detail shell and two selected conversation reads were real-device accepted on iPhone/iOS 17.0. PR #9 merge pending. Compact-width breadth beyond tested path remains Unverified. Not Frozen. |
...
| Native conversation read path | **Accepted Candidate** | `ConversationRepository` + sidebar/detail/message UI; `DEV-native-read-path-0.1.0-b9` | **Code + CI + Artifact + real-device tested** for shell/list/two-detail/current visible branch on iPhone/iOS 17.0. Position 1: 1.53 MB / 337 mapping / 154 visible / 5.67 s; position 13: 7.50 MB / 2023 mapping / 843 visible / 20.74 s. Terminal manual reload was not triggered; its failure-path runtime behavior remains Unverified. PR #9 merge pending. |
...
- `DEV-native-read-path-0.1.0-b9`: **Code written + CI passed + Artifact produced + Runtime/manual/real-device tested** for native shell/list/two-detail/current-branch message rendering; accepted candidate awaiting PR #9 merge. Terminal detail reload and install/update auth persistence remain separately Unverified.
