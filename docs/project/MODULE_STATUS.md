...
| Diagnostics / logging | Stable baseline + safe hybrid fields | `DiagnosticsLogger` | Hybrid diagnostics use route class / targetMatch / lifecycle timing only; no prompt/body/raw ID/auth/challenge values. |
...
| User-visible official-Web Send surface | **Feasibility accepted; full-page form rejected; background gate active** | `AuthWebViewController.hybridChat`; TD-024/TD-025 | b43 showed visible Web can be smooth/resident enough. b44 showed full-page Native->Web->Native duplicates loading and cannot immediately reconcile assistant output. Any next visible-Web product form must first pass `HYBRID_WEB_BACKGROUND_RESILIENCE_PLAN.md`. |
...
| Streaming / Send | **Blocked — existing-account path now background-resilience-gated** | `DEV-send-stream`; PR #29 | Pure-native ChatGPT-account Send remains blocked by b42 browser-challenge evidence. API product route explicitly rejected by user. Existing-account visible-Web direction may continue only if exact-device background/lock survival or transparent foreground recovery is proven. No b45 allocated. |
| Background execution / completion | **Plan active; hybrid-Web feasibility now prerequisite** | `BACKGROUND_EXECUTION_PLAN.md` + `HYBRID_WEB_BACKGROUND_RESILIENCE_PLAN.md` | Public `beginBackgroundTask` is finite-only baseline. TrollStore process preservation exists as a planned experiment, but main-app survival does not prove WebKit WebContent/Network stream survival. Exact iPhone/iOS17 Runtime required before accepting hybrid Send architecture. |
...
- API product architecture is no longer an active product option unless the user explicitly reverses that decision.
- Existing-account visible-Web Send is not accepted until the primary-device background/lock matrix proves either reliable live Web continuation or deterministic no-manual-refresh foreground recovery.
- Main app process survival alone is not sufficient evidence; WebKit page/process/stream survival must be distinguished.
- b39-b44 identities are permanently reserved; no b45 allocated.
- Still Unverified where applicable: lower iOS/iPad, non-personal workspace/account switch, supported Native attachment upload/handoff, immediate Native response ownership for Web Send, and TrollStore WebKit background continuity.
