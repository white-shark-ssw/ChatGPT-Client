...
- **Batch I0 — this record**: write only this `DEV-send-stream` checkpoint recovery point. No product, Candidate, Artifact, PR #36, PR #29 metadata, or other task checkpoint changes.
- **Batch I1 — pending**: re-read actual branch/PR state after I0 because the recorder commit advances the base by docs/tooling only. Require PR #36 head still `d5d761bf...`, mergeable, and product-equivalent to canonical package source `75ccad...`.
- **Batch I2 — pending**: merge PR #36 into `dev/send-stream-20260829` using expected head `d5d761bfad26bc90953488ccd5a96452bf356b3a`. This is integration of already-tested b113; it does not allocate a new `DEV-send-stream` candidate and does not create new Runtime evidence.
...
**Next exact action:** after verifying this recovery-point commit, re-check PR #36 against the docs-only-advanced base and merge it with expected head `d5d761bfad26bc90953488ccd5a96452bf356b3a`; then perform only missing I3/I4 work.
