...
- **Working branch / PR / head commit**: `dev/official-sync-reload-20260904`; PR not created; branch created from exact main baseline above. Head will advance with this checkpoint commit.
...
- **Files / modules in scope**: this checkpoint; future task-local `scripts/research/official_ios_sync_reload/**`; if needed, one task-local research workflow with a unique path/name. No product source changes are authorized yet.
