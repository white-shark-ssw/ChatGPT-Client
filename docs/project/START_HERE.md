...
7. Before multi-conversation, production send/stream, background-completion or TrollStore true-background work, also read `CLIENT_ARCHITECTURE_GAP_REVIEW.md` plus the relevant `MULTI_CONVERSATION_STATE_PLAN.md` / `BACKGROUND_EXECUTION_PLAN.md`. Before `DEV-send-stream` specifically, also read `SEND_STREAM_PREFLIGHT.md`; it records the current evidence gates for Send/new-chat/stream/Stop, pending-to-authoritative identity handoff, response ownership, follow-tail, conflict checks, diagnostics and real-device acceptance. These documents contain pre-send concurrency, async-freshness, account-scope and background active-response-set invariants that are not safe to rediscover ad hoc.
