...
**Active test candidate.** `DEV-conversation-recovery-0.1.0-b10`, version `0.1.0 (10)`, exact product/config source `89129913cb29a35db9dec7a6d5670d1b3b76bc23`, has reached **Code written + CI passed + Artifact produced** on PR #10. CI run `32982836557`; artifact ID `9612167843`; IPA SHA-256 `6e600f829fa24cdeb705e9ab104ebb780a8c70dd06871285d06fa30521aecb7e`. Runtime/manual/real-device recovery behavior remains Unverified, so b10 is not Stable or accepted yet.
...
Current b10 implementation preserves the previously loaded detail if the refresh fails and replaces the authoritative selected detail with the fresh server-backed current branch on success.
...
Current b10 implementation explicitly clears the selected authoritative detail before issuing the fresh detail request, then rebuilds from returned server state. Terminal `重新加载` routes through the same full-reload operation, while a loaded conversation exposes `重载当前会话` in the overflow menu.

- Preserve unsent composer draft when practical; no composer exists yet in the current read-only shell.
- Do not resend messages.
- No automatic retry/watchdog/timer/fallback path is added.
...
b10 records previous/current visible counts, added/removed/changed visible counts and local-state transitions while reusing the existing privacy-safe conversation hash/list-position evidence.

### Acceptance gate

Real-device testing must exercise both loaded-state actions and, where practical, the terminal reload path. CI/Artifact success is not runtime proof. If accepted, update durable runtime evidence and merge PR #10 before starting the serial `DEV-conversation-round-count` task.
...
The exact Swift property/type name is not frozen by planning; implementation must inspect current source and use the existing model/state structure.
...
Real-device test `DEV-conversation-recovery-0.1.0-b10` for loaded-state `同步最新消息`, loaded-state `重载当前会话`, and terminal reload behavior where practical. If accepted, record runtime evidence and merge PR #10. Then start the serial `DEV-conversation-round-count` task.
