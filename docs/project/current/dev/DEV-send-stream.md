...
`docs/project/BUILD_TEST_INDEX.md` has been updated: b83 is Runtime Rejected and b84 is permanently reserved / Runtime Pending. PR #29 has been updated to `b83 rejected — b84 Detail projection diagnostic Runtime next`.
...
Install exact b84. Start a deliberately long cross-platform response. While it is still generating, press manual Sync two or three times at separated points; b84 is diagnostic-only and is not expected to fix the UI. Export diagnostics while the response is still active if possible and provide the JSON.
