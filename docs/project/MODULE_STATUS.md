...
| Manual conversation recovery | **Stable / merged baseline + multi-conversation recovery Runtime evidence** | `ConversationRepository` + detail UI + shell | b17 Sync A->B->A rejoined same active Sync. b18 visible Sync/Reload anchor preservation and active Sync return coalescing accepted. b19-b21 do not change recovery behavior. Multi-conversation isolated Reload-replacement-under-load spot-check remains open. Not Frozen. |
| Multi-conversation state ownership | **Active / b17 core + b18 scroll + b19 memory + b21 title Runtime accepted; not Stable** | `DEV-multi-conversation-state`; branch `dev/multi-conversation-state-20260827`; b21 | Exact b21 source `6b50ead167bfde305d2ad58dd16fee6edaabf597`; Artifact `9645439329`. Direct user real-device testing accepts the requested b21 first-entry/re-entry/rapid-switch title matrix. Natural failure, supported account-switch, normal LRU headroom decision and one isolated replacement gate remain open. |
...
- b21 multi-conversation = **Code + static/source + CI + identity-valid Artifact + real-device title matrix accepted for tested iPhone/iOS17 scope**.
