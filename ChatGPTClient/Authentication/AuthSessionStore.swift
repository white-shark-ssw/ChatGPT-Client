        ...
        cookieStore.getAllCookies { [weak self] cookies in
            ...
            session.dataTask(with: URLRequest(url: Self.sessionURL)) { [weak self] data, response, error in
                ...
                guard let response = response as? HTTPURLResponse, let data, (200..<300).contains(response.statusCode) else {
                    ...
                    self.finishAccountProbe(state, span: span, fields: ["stage": "session", "httpStatus": statusCode.map(String.init) ?? "none"], completion: completion)
                    return
                }
                ...
                session.dataTask(with: request) { [weak self] data, response, error in
                    ...
                    guard let response = response as? HTTPURLResponse, let data, (200..<300).contains(response.statusCode) else {
                        let statusCode = (response as? HTTPURLResponse)?.statusCode
                        let state: AuthAccountContextState = statusCode == 403 ? .failed : .notAvailable
                        self.finishAccountProbe(state, span: span, fields: ["stage": "accounts", "httpStatus": statusCode.map(String.init) ?? "none"], completion: completion)
                        return
                    }
